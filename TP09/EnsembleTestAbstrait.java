 ...
 *  @author	Xavier Crégut &lt;prenom.nom@enseeiht.fr&gt;
  */
abstract public class EnsembleTestAbstrait<T> {
	...
	public void testAjouterPremier() {
		assertTrue(e1.estVide());
		e1.ajouter(getElement(10));
		assertFalse(e1.estVide());
		assertEquals(1, e1.cardinal());
		assertTrue(e1.contient(getElement(10)));
	}
	...
	public void testAjouterTroisElements() {
		e1.ajouter(getElement(10));
		e1.ajouter(getElement(15));
		e1.ajouter(getElement(-5));
		assertEquals(3, e1.cardinal());
		assertTrue(e1.contient(getElement(10)));
		assertTrue(e1.contient(getElement(-5)));
		assertTrue(e1.contient(getElement(15)));
	}
	...
	public void testContient() {
		...
		assertTrue(e1.contient(getElement(-5)));
		assertTrue(e1.contient(getElement(15)));
		assertFalse(e1.contient(getElement(1)));
		assertFalse(e1.contient(getElement(2)));
		assertFalse(e1.contient(getElement(100)));
		assertFalse(e1.contient(getElement(0)));
	}
	...
	public void testSupprimerPresent() {
		...
		assertEquals(3, e1.cardinal());
		assertTrue(e1.contient(getElement(10)));
		e1.supprimer(getElement(10));
		assertFalse(e1.contient(getElement(10)));
	...
	public void testSupprimerDifferentsCas() {
		...
		e1.supprimer(getElement(10));
		assertFalse(e1.contient(getElement(10)));
		e1.supprimer(getElement(-5));
		assertFalse(e1.contient(getElement(10)));
		assertFalse(e1.contient(getElement(-5)));
		assertTrue(e1.contient(getElement(15)));
		assertFalse(e1.contient(getElement(1)));
		assertFalse(e1.contient(getElement(2)));
		assertEquals(1, e1.cardinal());

		e1.supprimer(getElement(15));
		assertFalse(e1.contient(getElement(10)));
		assertFalse(e1.contient(getElement(-5)));
		assertFalse(e1.contient(getElement(15)));
		assertFalse(e1.contient(getElement(1)));
		assertFalse(e1.contient(getElement(2)));
		assertEquals(0, e1.cardinal());
		assertTrue(e1.estVide());
	...
	protected void testerAvecTab(int... tab) {
		...
		for (int i = 0; i < tab.length; i++) {
			e1.ajouter(getElement(tab[i]));
			assertEquals(i+1, e1.cardinal());
			// vérifier la présence des éléments insérés
			for (int j = 0; j <= i; j++) {
				assertTrue(e1.contient(getElement(tab[j])));
			}
			// vérifier l'absence des éléments à insérer
			for (int j = i+1; j < tab.length; j++) {
				assertFalse(e1.contient(getElement(tab[j])));
			}
		}
	}

	protected abstract T getElement(int value);
